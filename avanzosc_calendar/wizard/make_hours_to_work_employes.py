...
class MakeHoursToWorkEmployes(orm.TransientModel):
    ...
    _columns = {
        ...
        # Lineas
        'make_hours_to_work_employes_line_ids':
            fields.one2many('make.hours.to.work.employes.line',
                            'make_hours_to_work_employes_id', 'Employes'),
    }
            ...
            for template_employe in wiz.make_hours_to_work_employes_line_ids:
                if template_employe.selected:
                    ...
                    while fec_ini <= fec_fin:
                        if (monday or tuesday or wednesday or thursday or
                                friday or saturday or sunday):
                            dia = self._calculate_day(cr, uid, fec_ini)
                            if monday and dia == 'Lunes':
                                self._write_hours_to_work(
                                    cr, uid, template_employe.employee_id.id,
                                    fec_ini, name, hours, background_color)
    ...
    def _write_hours_to_work(self, cr, uid, employee_id, my_date, name, hours,
        ...
        if not employ_calendar_ids:
            ...
            hr_employee_calendar_id = employ_calendar_obj.create(cr, uid,
                                                                 line_vals)
        else:
            hr_employee_calendar_id = employ_calendar_ids[0]
        cond = [('hr_employee_calendar_id', '=', hr_employee_calendar_id),
                ('date', '=', actual_date)]
