...
class ImportTemplateEmployes(orm.TransientModel):
    ...
    _columns = {
        ...
        # Lineas
        'import_template_employes_line_ids':
            fields.one2many('import.template.employes.line',
                            'import_template_employes_id', 'Employes'),
    }
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        for employee in employee_obj.browse(cr, uid, data, context=context):
    ...
    def action_import_template_employes(self, cr, uid, ids, context=None):
        ...
        calendar_resources_obj = self.pool['estimated.calendar.resources']
                    ...
                    while fec_ini <= fec_fin:
                        ...
                        if not festive_tline_ids:
                            cond = [('hr_employee_calendar_id', '=',
                                     employ_calendar_id),
                                    ('date', '=', my_date)]
                            resources_ids = calendar_resources_obj.search(
                                cr, uid, cond, context=context)
                            if not resources_ids:
                                ...
                                calendar_resources_obj.create(
                                    cr, uid, line_vals, context=context)
                            else:
                                line_vals = {'name': False,
                                             'hours': 8}
                                calendar_resources_obj.write(
                                    cr, uid, resources_ids, line_vals,
                                    context=context)
                        else:
                            festive_template = festive_tline_obj.browse(
                                cr, uid, festive_tline_ids[0],
                                context=context)
                            cond = [('hr_employee_calendar_id', '=',
                                     employ_calendar_id),
                                    ('date', '=', my_date)]
