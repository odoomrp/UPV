...
class SimulationCategory(orm.Model):
    _name = 'simulation.category'
    _description = 'Simulation Category'
    ...
    _columns = {
        ...
        # Area de gasto
        'expense_area_id':
            fields.many2one('expense.area', 'Expense Area', required=True),
        # Categoria
        'category_id':
    ...
    def onchange_restricted_category(self, cr, uid, ids, category_id,
        ...
        if my_products:
            ...
            result.update({'warning': {'title': 'warning', 'message':
                                       _('Found products with this restricted'
                                         ' category, products: ') + texto}})
        return result
